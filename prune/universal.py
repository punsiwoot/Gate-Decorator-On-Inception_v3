...
class GatedBatchNorm2d(Meltable):
    def __init__(self, bn, minimal_ratio = 0.1):
        ...
        self.g = nn.Parameter(torch.ones(1, self.channel_size, 1, 1).to(self.device), requires_grad=True)  ## create a gate for importace score
        self.register_buffer('area', torch.zeros(1).to(self.device))
        self.register_buffer('score', torch.zeros(1, self.channel_size, 1, 1).to(self.device))  ## create score storage
    ...
    def start_collecting_scores(self):
        ...
        self._hook = self.g.register_hook(self.cal_score)  #register_hook is hook a gradient of parameter g and also passing into cal_score function and keep it to self._hook
...
class Conv2dObserver(Meltable):
    ...
    def _backward_hook(self, grad):
        ...
        new_grad = torch.ones_like(grad)          ## receive a gradient, save and turn to one
        return new_grad

    def forward(self, x):
        output = self.conv(x)
        noise = torch.zeros_like(output).normal_()
        output = output + noise                             ## add noise to the data ??
        if self.training:
    ...
    def melt(self):
        ...
        replacer = nn.Conv2d(
            in_channels = int((self.in_mask != 0).sum()),    ## get the new cnn when remove filter
